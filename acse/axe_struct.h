...
typedef struct t_axe_variable
{
   ...
   int location;  /* register ID which at runtime contains the value of
                   * scalar variables. Not used if isArray is true. */
   t_axe_label *labelID;   /* label that points to the memory allocated to
                            * this array inside the data segment. Not used if
                            * isArray is false. */
} t_axe_variable;
...
typedef struct t_axe_optimized_and {
   t_axe_label *end_expr;
   int res_reg;
} t_axe_optimized_and;
...
extern t_while_statement create_while_statement(void);
