...
char *names[4][16] = {
      {"ADD", "SUB", "ANDL", "ORL", "EORL", "ANDB", "ORB", "EORB", "MUL", "DIV",
            "SHL", "SHR", "ROTL", "ROTR", "NEG", "SPCL"},
      {"ADDI", "SUBI", "ANDLI", "ORLI", "EORLI", "ANDBI", "ORBI", "EORBI",
            "MULI", "DIVI", "SHLI", "SHRI", "ROTLI", "ROTRI", "NOTL", "NOTB"},
      {"NOP", "MOVA", "JSR", "RET", "LOAD", "STORE", "HALT", "SEQ", "SGE",
            "SGT", "SLE", "SLT", "SNE", "READ", "WRITE", "XPSW"},
      {"BT", "BF", "BHI", "BLS", "BCC", "BCS", "BNE", "BEQ", "BVC", "BVS",
...
decoded_instr *decode(int the_instr)
{
   ...
   instr->func = getbits(the_instr, 10, 0);    /* 11 extra bits, used to encode
                                                  extra info */
   ...
   switch (instr->format) {
      ...
      case FORMAT_BIN:
         fprintf(file, "%s R%d, R%d, #%d \n",
               names[instr->format][instr->opcode], instr->dest, instr->src1,
               instr->imm);
         break;
      case FORMAT_UNR:
      ...
      case FORMAT_CC:
         ...
         break;
      default: fprintf(file, "[UNKNOWN INSTRUCTION FORMAT] \n"); break;
   }
}
